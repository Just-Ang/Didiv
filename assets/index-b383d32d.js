...
`,SA=({item:e})=>{const t=jn();return f.jsxs(wA,{children:[f.jsx("button",{onClick:()=>t(W4(e.id)),children:"-"}),f.jsx("span",{children:e.quantity}),f.jsx("button",{onClick:()=>t(H4({id:e.id,stock:e.stock})),disabled:e.quantity>=e.stock,children:"+"})]})},_A=()=>{const e=jn(),t=On(),[n,r]=b.useState([]),i=Ye(p=>p.cart.items),a=Ye(p=>p.cart.items).reduce((p,m)=>p+m.quantity,0),l=i.reduce((p,m)=>p+(m.new_price??m.price)*(m.quantity||1),0),s=Ye(p=>p.favorites.items),u=i.length===0,d=(p,m)=>{m.stopPropagation(),console.log(p);const g=s.some(w=>w.id===p.id);e(ai(p)),g?q.warning(`${p.name} видалено з обраного`):q.info(`${p.name} додано в обране`)},c=p=>{r(m=>[...m,p.id]),setTimeout(()=>{e(V4(p)),r(m=>m.filter(g=>g!==p.id))},300)},y=()=>{e(am())};return f.jsx(f.Fragment,{children:u?f.jsx(bA,{}):f.jsxs(KL,{children:[f.jsx(Or,{autoClose:1500}),f.jsxs(QL,{children:[" ",f.jsx(Oe,{to:"/",style:{color:"inherit",textDecoration:"none"},children:"Головна"})," ","/"," ",f.jsx(Oe,{to:"/cart",style:{color:"inherit",textDecoration:"none"},children:"Кошик"})]}),f.jsx(ZL,{children:"Кошик"}),f.jsxs(JL,{children:[f.jsx(eA,{children:i.map((p,m)=>{var x,S;console.log(p);const g=s.some(C=>C.id===p.id),w=p.new_price&&p.new_price<p.price,h=w?p.new_price:p.price,v=w?Math.round((p.price-p.new_price)/p.price*100):0;return f.jsxs(tA,{className:n.includes(p.id)?"removing":"",children:[f.jsx(nA,{src:((S=(x=p.images)==null?void 0:x[0])==null?void 0:S.url)||"/nofoto.png",alt:p.name,onClick:()=>t(`/product/${p.id}`),onError:C=>{C.currentTarget.onerror=null,C.currentTarget.src=no}}),f.jsx(rA,{children:f.jsx("h3",{children:p.name})}),f.jsxs(iA,{children:[f.jsx(SA,{item:{...p,quantity:p.quantity||1}}),f.jsx(wc,{children:f.jsxs(Sc,{children:[f.jsxs(_c,{$discount:w,children:[(h*(p.quantity||1)).toLocaleString()," грн"]}),w&&f.jsxs(f.Fragment,{children:[f.jsxs(Cc,{children:[(p.price*(p.quantity||1)).toLocaleString()," грн"]}),f.jsxs(Ec,{children:["-",v,"%"]})]})]})})]}),f.jsxs(oA,{children:[f.jsx(sA,{onClick:C=>d(p,C),style:{background:"none",border:"none",cursor:"pointer",color:"#ccc"},children:f.jsx(Ua,{size:22,fill:g?"#ff4d4f":"none",color:g?"#ff4d4f":"#999"})}),f.jsx(aA,{onClick:()=>c(p),style:{background:"none",border:"none",cursor:"pointer",color:"#000000"},children:f.jsx(Ub,{size:22})})]})]},`${p.id}-${m}`)})}),f.jsxs(lA,{children:[f.jsxs(uA,{children:[f.jsx("span",{children:"Всього в обраному:"}),f.jsxs("strong",{children:[a," шт."]}),f.jsx("span",{children:"На суму:"}),f.jsxs("strong",{children:[l," грн"]})]}),f.jsx(cA,{to:"/checkout",children:"Оформити замовлення"}),f.jsx(dA,{onClick:y,children:"Oчистити кошик"})]})]})]})})},CA=E.div`
  color: var(-black-color);
  font-family: var(--main-font);
  width: 100%;
  max-width: 750px;
