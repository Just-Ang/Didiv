...
`,dR=({item:e})=>{const t=Jt();return f.jsxs(cR,{children:[f.jsx("button",{onClick:()=>t(D4(e.id)),children:"-"}),f.jsx("span",{children:e.quantity}),f.jsx("button",{onClick:()=>t(I4({id:e.id,stock:e.stock})),disabled:e.quantity>=e.stock,children:"+"})]})},fR=()=>{const e=Jt(),t=Bt(),[n,r]=x.useState([]),i=Ue(p=>p.cart.items),a=Ue(p=>p.cart.items).reduce((p,m)=>p+m.quantity,0),l=i.reduce((p,m)=>p+(m.new_price??m.price)*(m.quantity||1),0),s=Ue(p=>p.favorites.items),u=i.length===0,d=(p,m)=>{m.stopPropagation(),console.log(p);const g=s.some(w=>w.id===p.id);e(ci(p)),g?Y.warning(`${p.name} видалено з обраного`):Y.info(`${p.name} додано в обране`)},c=p=>{r(m=>[...m,p.id]),setTimeout(()=>{e(M4(p)),r(m=>m.filter(g=>g!==p.id))},300)},y=()=>{e(vm())};return f.jsx(f.Fragment,{children:u?f.jsx(uR,{}):f.jsxs(zA,{children:[f.jsx(Dn,{autoClose:1500}),f.jsxs(NA,{children:[" ",f.jsx(Pe,{to:"/",style:{color:"inherit",textDecoration:"none"},children:"Головна"})," ","/"," ",f.jsx(Pe,{to:"/cart",style:{color:"inherit",textDecoration:"none"},children:"Кошик"})]}),f.jsx(BA,{children:"Кошик"}),f.jsxs(VA,{children:[f.jsx(UA,{children:i.map((p,m)=>{var b,S;console.log(p);const g=s.some(E=>E.id===p.id),w=p.new_price&&p.new_price<p.price,h=w?p.new_price:p.price,v=w?Math.round((p.price-p.new_price)/p.price*100):0;return f.jsxs(WA,{className:n.includes(p.id)?"removing":"",children:[f.jsx(HA,{src:((S=(b=p.images)==null?void 0:b[0])==null?void 0:S.url)||"/nofoto.png",alt:p.name,onClick:()=>t(`/product/${p.id}`),onError:E=>{E.currentTarget.onerror=null,E.currentTarget.src=ii}}),f.jsx(GA,{children:f.jsx("h3",{children:p.name})}),f.jsxs(qA,{children:[f.jsx(dR,{item:{...p,quantity:p.quantity||1}}),f.jsx(jc,{children:f.jsxs(Mc,{children:[f.jsxs($c,{$discount:w,children:[(h*(p.quantity||1)).toLocaleString()," грн"]}),w&&f.jsxs(f.Fragment,{children:[f.jsxs(Ic,{children:[(p.price*(p.quantity||1)).toLocaleString()," грн"]}),f.jsxs(Dc,{children:["-",v,"%"]})]})]})})]}),f.jsxs(YA,{children:[f.jsx(KA,{onClick:E=>d(p,E),style:{background:"none",border:"none",cursor:"pointer",color:"#ccc"},children:f.jsx(Ya,{size:22,fill:g?"#ff4d4f":"none",color:g?"#ff4d4f":"#999"})}),f.jsx(XA,{onClick:()=>c(p),style:{background:"none",border:"none",cursor:"pointer",color:"#000000"},children:f.jsx(mw,{size:22})})]})]},`${p.id}-${m}`)})}),f.jsxs(QA,{children:[f.jsxs(ZA,{children:[f.jsx("span",{children:"Всього в обраному:"}),f.jsxs("strong",{children:[a," шт."]}),f.jsx("span",{children:"На суму:"}),f.jsxs("strong",{children:[l," грн"]})]}),f.jsx(JA,{to:"/checkout",children:"Оформити замовлення"}),f.jsx(eR,{onClick:y,children:"Oчистити кошик"})]})]})]})})},pR=_.div`
  color: var(-black-color);
  font-family: var(--main-font);
  width: 100%;
  max-width: 750px;
